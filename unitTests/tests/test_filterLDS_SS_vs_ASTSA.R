...
processAll <- function() {
    ...
    srQ <- chol(x=Gamma0)
    srR <- chol(x=Sigma0)
    fRes_filterLDS_SS <- filterLDS_SS(B=A0, Z=C0, x0=x0, V0=V0, stateType0=stateType0, Q=srQ%*%t(srQ), R=srR%*%t(srR), ys=obs)
    fRes_Kfilter0 <- Kfilter0(num=ncol(obs), y=t(obs), A=C0, mu0=x0, Sigma0=V0, Phi=A0, cQ=srQ, cR=srR)

    for(n in 1:nObs) {
        diff <- fRes_filterLDS_SS$xnn[, 1, n]-fRes_Kfilter0$xf[, 1, n]
        error <- l2Norm(diff)
        if(error>tol) {
