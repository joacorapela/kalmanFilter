...
processAll <- function() {
    ...
    fRes <- filterLDS_SS(y=testSimRes$y, B=estRes$B, Z=estRes$Z, m0=estRes$m0, V0=estRes$V0, Q=estRes$Q, R=estRes$R)
    meanOneStepAheadForecast <- estRes$Z%*%fRes$xnn1
    covOneStepAheadForecast <- estRes$Z%*%fRes$Vnn1%*%t(estRes$Z)+estRes$R
    fRes0 <- filterLDS_SS(y=testSimRes$y, B=initialConds$B, Z=initialConds$Z, m0=initialConds$m0, V0=initialConds$V0, Q=initialConds$Q, R=initialConds$R)
    meanOneStepAheadForecast0 <- initialCond$Z%*%fRes0$xnn1
    covOneStepAheadForecast0 <- initialCond$Z%*%fRes0$Vnn1%*%t(initialCond$Z)+initialCond$R

    trueLinetype <- "solid"
    initialLinetype <- "dot"
    estLinetype <- "dash"
    cols <- brewer.pal(max(3, nrow($x)), "Set1")
    fig <- plot_ly(type='scatter', mode='lines')
    for(i in 1:nrow(testSimRes$y)) {
        fig <- fig%>%add_trace(x=1:ncol(testSimRes[i,]), y=simRes$x[i,], name=sprintf("true[,%d]", i), line=list(color=cols[i], dash=trueLinetype))
    }
...
#     pngFigFilename <- sprintf(figFilenamePattern, estNumber, "png")
#     htmlFigFilename <- sprintf(figFilenamePattern, estNumber, "html")
