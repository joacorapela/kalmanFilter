...
loadRes = load('results/simulation2DTrajectory.mat');
% Initialize simulation variables
SRSigmaW = chol(loadRes.Gamma,'lower'); % Square-root process noise covar
SRSigmaV = chol(loadRes.Sigma,'lower'); % Square-root sensor noise covar
A = loadRes.A; B = zeros(size(A,1), 1); C = loadRes.C; D = zeros(size(C,1), 1); % Plant definition matrices
N = size(loadRes.x, 2);
xHat0 = loadRes.mu0; % Initialize estimated system initial state
SigmaX0 = loadRes.V0; % Initialize Kalman filter initial covariance
...
plot(xs(1,:), xs(2,:), xHats(1,:), xHats(2,:))
legend('x', 'xHat')
