...
processAll <- function() {
    ...
    A0 <- initialConds$A+rnorm(n=length(initialConds$A))*300.0
    # A0 <- A
    Gamma0 <- gamma0Var*diag(rep(1, times=nrow(A0)))
    ...
    fRes <- squareRootKF(A=eRes$A, B=B, C=eRes$C, D=D, x0=eRes$x0, SRSigmaX0=chol(x=eRes$V0), SRSigmaW=chol(x=eRes$Gamma), SRSigmaV=chol(x=eRes$Sigma), us=us, zs=zs)
    sRes <- smoothLDS(A=eRes$A, mu=fRes$x, V=fRes$SigmaXHat, P=fRes$SigmaX[2:length(fRes$SigmaX)])
    fRes0 <- squareRootKF(A=A0, B=B0, C=C0, D=D0, x0=xHat00, SRSigmaX0=chol(x=V00), SRSigmaW=chol(x=Gamma0), SRSigmaV=chol(x=Sigma0), us=us, zs=zs)
