...
processAll <- function() {
    ...
    filterResFigFilename <- "figures/srFilterResPendulum.png"
    ...
    df <- data.frame(x=c(simRes$t, simRes$t, simRes$t, simRes$t, simRes$t), y=c(simRes$x[1,], simRes$z[1,], simRes$z[2, ], filterRes$xHat[1,], filterRes$xHat[2,]), type=factor(c(rep("measurement", nObs), rep("latent acceleration", nObs), rep("latent position", nObs), rep("filtered acceleration", nObs), rep("filtered position", nObs))))
    p <- ggplot()
    p <- p + geom_point(data=df, aes(x=x, y=y, colour=type))
    p <- p + scale_colour_manual(values=c("measurement"="blue", "latent acceleration"="lightgreen", "latent position"="darkgreen", "filtered acceleration"="orangered", "filtered position"="darkred"))
